defmodule MimeHeadersTest do
  ...
  test "decoded params with quoted string" do
    assert %{withoutquote: "with no quote", withquote: " with; some \"quotes\" "} =
             MimeMail.Params.parse_header(
               "   withoutquote =  with no quote  ; WithQuote =\" with; some \\\"quotes\\\" \""
             )
  end
  ...
  test "encode addresses headers" do
    mail = %MimeMail{
      headers: [
        to: [
          %MimeMail.Address{address: "frnog@frnog.org"},
  ...
  test "round trip encoded-words" do
    assert "Jérôme Nicolle gave me €" =
             "Jérôme Nicolle gave me €"
             |> MimeMail.Words.word_encode()
             |> MimeMail.Words.word_decode()
  end
  ...
  test "decode str from q-encoded-word" do
    assert "[FRnOG] [TECH] ToS implémentée chez certains transitaires" =
             MimeMail.Words.word_decode(
               "[FRnOG] =?UTF-8?Q?=5BTECH=5D_ToS_impl=C3=A9ment=C3=A9e_chez_certa?=\r\n =?UTF-8?Q?ins_transitaires?="
             )
  end
