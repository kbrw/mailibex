defmodule FlatMailTest do
  ...
  test "unflat mail with attachments only" do
    mail =
      MimeMail.Flat.to_mail(
        txt: "coucou arnaud",
        attach: "du texte attaché",
        attach: File.read!("test/mimes/sample.7z")
      )
    ...
    assert {"attachment", %{filename: cd_txt_name}} =
             text_attached.headers[:"content-disposition"]
  ...
  test "flat mail mixed(alternative(txt,html))" do
    flat =
      File.read!("test/mails/amazon.eml")
  ...
  test "flat mail alternative(txt,related(html,img))" do
    ...
    assert [{:txt, txt}, {:html, html}, {:include, {"imglogo", "image/png", png}} | _headers] =
             flat
