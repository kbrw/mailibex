defmodule DKIM do
  defstruct h: [:to, :from, :date, :subject, :"message-id", :"mime-version"],
            c: %{header: :relaxed, body: :simple},
            d: "example.org",
            s: "default",
            a: %{sig: :rsa, hash: :sha256},
            b: "",
            bh: "",
            l: nil,
            v: "1",
  ...
  def check(mail) when is_binary(mail), do: check(MimeMail.from_string(mail))
                ...
                case extract_key64(pubkey[:p] || "") do
                  {:ok, key} ->
                    ...
                    if :crypto.verify(:rsa, :sha256, header_h, sig.b, key) do
                      ...
                      {:permfail, :sig_not_match}
                    end

                  :error ->
                    {:permfail, :invalid_pub_key}
                end
              else
                {:permfail, :sig_algo_not_match}
              end
  ...
  def sign(mail, key, sig_params \\ []) do
    ...
    %{body: {:raw, body}} = encoded_mail = MimeMail.encode_body(mail)
    # add body hash
    sig = %{sig | bh: body_hash(body, sig)}
    # encoded mail without dkim.b
    encoded_mail =
      MimeMail.encode_headers(%{
        encoded_mail
        | headers: Keyword.put(encoded_mail.headers, :"dkim-signature", sig)
      })
    ...
    case headers[:"dkim-signature"] do
      {:raw, raw} ->
        ...
        sig =
          struct(
            DKIM,
            for({k, v} <- MimeMail.Params.parse_header(unquoted), do: {k, decode_field(k, v)})
          )
  ...
  defp decode_field(:h, h),
    do: h |> String.downcase() |> String.split(":") |> Enum.map(&String.to_atom/1)
  ...
  def canon_body(body, :simple), do: String.replace(body, ~r/(\r\n)*$/, "\r\n", global: false)
  ...
  def extract_key(
        <<48, 1::size(1)-unit(1), ll0::size(7), _l0::size(ll0)-unit(8), 48, l1,
          _algoid::size(l1)-binary, 3, 1::size(1)-unit(1), ll2::size(7), l2::size(ll2)-unit(8),
          des_rsapub::size(l2)-binary>>
      ) do
  ...
  def extract_key(
        <<48, 1::size(1)-unit(1), ll0::size(7), _l0::size(ll0)-unit(8), 2, 1::size(1)-unit(1),
          ll1::size(7), l1::size(ll1)-unit(8), mod::size(l1)-binary, 2, l2,
          exp::size(l2)-unit(8)-binary>>
      ) do
    {:ok, [exp, mod]}
  end

  # strip leading 0 if needed
  def extract_key(<<0, rest::binary>>), do: extract_key(rest)
...
defimpl MimeMail.Header, for: DKIM do
  ...
  defp encode_field(:b, b),
    do: Base.encode64(b) |> chunk_hard([]) |> Enum.join("\r\n            ")

  defp encode_field(_, e), do: Kernel.to_string(e)

  defp chunk_hard(<<vline::size(50)-binary, rest::binary>>, acc),
    do: chunk_hard(rest, [vline | acc])
