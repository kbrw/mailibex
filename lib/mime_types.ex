defmodule MimeTypes do
  ...
  {ext2mime, mime2ext} =
    case :httpc.request(
           ~c"https://svn.apache.org/repos/asf/httpd/httpd/trunk/docs/conf/mime.types"
         ) do
      {:ok, {{_, 200, _}, _, r}} -> "#{r}"
    ...
    |> Enum.reduce({%{}, []}, fn line, {ext2mime, mime2ext} ->
      ...
      {Enum.into(Enum.map(exts, fn ext -> {ext, mime} end), ext2mime),
  ...
  |> Enum.sort_by(&(&1 |> elem(0) |> byte_size()))
  |> Enum.reverse()
  |> Enum.each(fn {ext, mime} ->
  ...
  def path2mime(path), do: path |> Path.extname() |> String.downcase() |> ext2mime()
  ...
  def bin2ext(<<"BM", len::size(32)-little, _::binary>> = bin) when byte_size(bin) == len,
    do: ".bmp"
  ...
  def bin2ext(<<0b11111111111::size(11), mpeg::size(2), 0b01::size(2), _::size(1), _::binary>>)
      when mpeg in [0b11, 0b10], do: ".mp3"

  def bin2ext(<<0b11111111111::size(11), mpeg::size(2), 0b10::size(2), _::size(1), _::binary>>)
      when mpeg in [0b11, 0b10], do: ".mp2"

  def bin2ext(<<"MThd", 6::size(32), _::binary>>), do: ".mid"
  def bin2ext(<<"OggS", 0, 2, _::binary>>), do: ".ogg"
  # handle ASF only for wmv files (need parsing to find wma)
  def bin2ext(<<0x3026B2758E66CF11A6D900AA0062CE6C::size(128), _::binary>>), do: ".wmv"
  ...
  # in case of ebml file, parse it to retrieve file type
  def bin2ext(<<0x1A, 0x45, 0xDF, 0xA3, _::binary>> = bin) do
  ...
  def bin2ext(<<0x04034B50::size(32)-little, _::binary>> = zip) do
    ...
    res =
      cond do
        Enum.all?([~c"[Content_Types].xml", ~c"word/styles.xml"], &(&1 in files)) ->
          ".docx"

        Enum.all?([~c"[Content_Types].xml", ~c"xl/styles.xml"], &(&1 in files)) ->
          ".xlsx"

        Enum.all?([~c"[Content_Types].xml", ~c"ppt/presProps.xml"], &(&1 in files)) ->
          ".pptx"
        ...
        ~c"META-INF/MANIFEST.MF" in files ->
          ".jar"
  ...
  def bin2ext(
        <<48, 1::size(1)-unit(1), lenlen::size(7), len::size(lenlen)-unit(8),
          _::size(len)-binary>>
      ),
      do: ".der"
  ...
  # compound file is doc (do not handle .xls,.ppt)
  def bin2ext(<<0xD0CF11E0A1B11AE1::size(64), _::binary>>), do: ".doc"
  def bin2ext("BEGIN:VCARD\r\nVERSION:" <> _), do: ".vcf"
  def bin2ext("BEGIN:VCALENDAR" <> _), do: ".ics"
  ...
  def bin2ext(content), do: if(String.printable?(content), do: ".txt", else: ".bin")
end
...
defmodule EBML do
  ...
  defp convert(:integer, len, bin),
    do:
      (
        <<i::signed-size(len)-unit(8)>> = bin
        i
      )

  defp convert(:uinteger, len, bin),
    do:
      (
        <<i::unsigned-size(len)-unit(8)>> = bin
        i
      )

  defp convert(:float, len, bin),
    do:
      (
        <<f::float-size(len)-unit(8)>> = bin
        f
      )
  ...
  defp convert(:date, 8, <<since2001::signed-size(8)-unit(8)>>) do
    ts = 978_307_200 + div(since2001, 1_000_000_000)
    :calendar.now_to_datetime({div(ts, 1_000_000), rem(ts, 1_000_000), 0})
  end

  ebml_spec =
    case :httpc.request(
           ~c"https://raw.githubusercontent.com/Matroska-Org/foundation-source/master/spectool/specdata.xml"
         ) do
      {:ok, {{_, 200, _}, _, r}} -> "#{r}"
