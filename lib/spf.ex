defmodule SPF do
  ...
  def apply_rule(rule, params) do
    try do
      ...
      defaultfail =
        "domain of #{params.sender} does not designate #{:inet.ntoa(params.client_ip)} as permitted sender"
  ...
  def term_match("a" <> arg, params) do
    domain_spec =
      if match?(":" <> _, arg), do: String.trim_leading(arg, ":"), else: params.domain <> arg
    ...
    case :inet_res.gethostbyname(~c"#{domain}", family) do
      {:ok, {:hostent, _, _, _, _, ip_list}} ->
        if Enum.any?(ip_list, &ip_in_network(params.client_ip, &1, prefix)),
          do: :match,
          else: :notmatch
        ...
        case :inet_res.gethostbyname(name, family) do
          {:ok, {:hostent, _, _, _, _, ip_list}} ->
            if params.client_ip in ip_list do
              if String.ends_with?("#{name}", target_name(domain_spec, params)),
                do: :match,
                else: :notmatch
            else
              :notmatch
  ...
  def term_match(<<"ip", v, ":", addr_spec::binary>>, params) when v in [?4, ?6] do
    ...
    if (family == :inet and v !== ?4) or (family == :inet6 and v !== ?6) do
      :notmatch
    else
      ...
      case :inet.parse_address(~c"#{addr_spec}") do
        {:ok, addr}
        when (tuple_size(addr) == 4 and family == :inet) or
  ...
  def lookup_limit_reset, do: Process.put(:lookups, 0)
  ...
  def lookup_limit_exceeded do
    case Process.get(:lookups, 0) do
      ...
      count ->
        Process.put(:lookups, count + 1)
        false
    end
  end

  def extract_prefix(domain_spec, family) when is_binary(domain_spec),
    do: extract_prefix(String.split(domain_spec, "/"), family)

  def extract_prefix([domain, _, v6pref], :inet6), do: extract_prefix([domain, v6pref], :inet6)
  ...
  def extract_prefix([domain], :inet), do: {domain, 32}
  def extract_prefix([domain], :inet6), do: {domain, 128}

  defp bin_ip({ip1, ip2, ip3, ip4}),
    do: <<ip1::size(8), ip2::size(8), ip3::size(8), ip4::size(8)>>

  defp bin_ip({ip1, ip2, ip3, ip4, ip5, ip6, ip7, ip8}),
    do:
      <<ip1::size(16), ip2::size(16), ip3::size(16), ip4::size(16), ip5::size(16), ip6::size(16),
        ip7::size(16), ip8::size(16)>>
  ...
  def ip_in_network(addr, net_addr, bitprefix) do
    ...
    # mask is bitprefix*1 + (bitlen-bitprefix)*0
    mask = fullone <<< (bitlen - bitprefix)
    (mask &&& net_ip) == (mask &&& ip)
  end

  def target_name(name, params), do: target_name(name, params, [])

  def target_name("", _, acc), do: acc |> Enum.reverse() |> to_string()
  ...
  def target_name(<<"%{", macro, rest::binary>>, params, acc)
    ...
    expanded =
      case Integer.parse(String.downcase(transfo)) do
  ...
  def target_name("%%" <> rest, params, acc), do: target_name(rest, params, [?% | acc])
  def target_name("%_" <> rest, params, acc), do: target_name(rest, params, [?\s | acc])
  def target_name("%-" <> rest, params, acc), do: target_name(rest, params, ["%20" | acc])
  def target_name("%" <> _, _, _), do: throw(:wrongmacro)
  def target_name(<<c, rest::binary>>, params, acc), do: target_name(rest, params, [c | acc])
  ...
  def target_name_transfo(expanded, start_index, "r" <> delimiters),
    do: target_name_transfo(expanded, start_index, true, delimiters)

  def target_name_transfo(expanded, start_index, delimiters),
    do: target_name_transfo(expanded, start_index, false, delimiters)

  def target_name_transfo(expanded, start_index, reversed?, ""),
    do: target_name_transfo(expanded, start_index, reversed?, ".")
